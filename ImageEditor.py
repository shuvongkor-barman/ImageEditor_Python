...
def img_rotate_custom():
    ...
    rotated_msg = str(img_custom_angel) + " Degree"
    scale = 1.0
...
def create_line():
    ...
    line_color_string = re.split(regex_pattern,
                                 simpledialog.askstring("User Input", "Please Enter Line color in RGB mode"))
...
def create_circle():
    ...
    circle_color_string = re.split(regex_pattern,
                                   simpledialog.askstring("User Input", "Please Enter Circle color in RGB mode"))
...
def create_rectangle():
    ...
    rectangle_pt2 = re.split(regex_pattern,
                             simpledialog.askstring("User Input", "Please EnterRectangle Point2: x2, y2"))
    ...
    while True:
        try:
            ...
            text_color_string = re.split(regex_pattern,
                                         simpledialog.askstring("User Input", "Please Enter Text color in RGB mode"))
            ...
            cv2.putText(img, your_text, text_pos, font, 3, text_color, text_weight, cv2.LINE_AA)
            edited_image = img
...
def save_image():
    ...
    if labelText.get() == "No Image":
        ...
        cv2.imwrite(os.path.join(select_location, just_file_name + "_Edited_" + str(save_count)) + get_format,
                    edited_image)
...
button_Open = HoverButton(root, text="Open File", image=button_open_icon_fit, compound=LEFT, font=('arial', 15, 'bold'),
                          bg='gray', fg='white',
                          activebackground='#3061f2', width=root.winfo_screenwidth(), command=openfile).pack(side=TOP)
# Image URL
...
button_Resize = HoverButton(root, text="Image Resize", image=button_resize_icon_fit, compound=LEFT, border=3, font=('arial', 12, 'bold'), activebackground='silver',
                            command=image_resize).pack(side=TOP)

# Create Button
create_selection.pack()
button_create = HoverButton(root, text="Create", border=3, font=('arial', 12, 'bold'), activebackground='silver',
                            command=image_create).pack(side=TOP)

button_put_text = HoverButton(root, text="Put Text in an Image", border=3, font=('arial', 12, 'bold'),
                              activebackground='silver', command=put_text).pack(side=TOP)
...
button_View = HoverButton(root, text="View", image=button_view_icon_fit, compound=LEFT, font=('arial', 12, 'bold'),
                          bg='#9db87f', fg='black', width=90,
                          activebackground='green', command=view_image).pack(side=TOP, pady=(15, 0))

separator_line2 = Label(root, text="_________________________________", font=('arial', 11, 'bold')).pack(side=TOP,
                                                                                                         pady=(0, 0))
